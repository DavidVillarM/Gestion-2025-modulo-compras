...
export default function FacturaForm() {
  ...
  useEffect(() => {
    if (form.ordenId) {
      const orden = ordenes.find(o => o.id === parseInt(form.ordenId));
      if (orden) {
        setForm(prev => ({ ...prev, proveedorId: orden.idProveedor }));
        setProductos(orden.detalles);
        setForm(prev => ({
          ...prev,
          items: orden.detalles.map(p => ({
